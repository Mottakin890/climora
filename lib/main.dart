...
Future<void> _setDeployment(Deployment deployment) async {
  ...
  try {
    EnvConfig.baseUrl = (deployment == Deployment.production)
        ? dotenv.env['PRODUCTION_BASE_URL']!
        : dotenv.env['STAGING_BASE_URL']!;

    EnvConfig.apiKey = (deployment == Deployment.production)
        ? dotenv.env['PRODUCTION_API_KEY']!
        : dotenv.env['STAGING_API_KEY']!;
    developer.log(
      "SetDeployment: ApiEndpoints configured for $deployment.",
      name: 'Main',
    );
  } catch (e) {
    developer.log(
      "SetDeployment Error while accessing .env: $e",
      name: 'Main',
      error: e,
    );
  }
...
Future<void> _init() async {
  ...
  final deployment = (EnvConfig.environment == 'production')
